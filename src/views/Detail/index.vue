        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车,在进行路由跳转之前,发送网络请求,将购买产品的信息通过请求的形式通知服务器,服务器进行相应的存储 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 处理购买产品个数的方法
      changeSkuNum(e){
        // 若产品的个数为负数或非number类型的数据,强制修改产品的个数为1
        if(isNaN(this.skuNum*1) || this.skuNum*1<0){
          this.skuNum = 1;
        }else{
      ...
      // 添加到购物车
      addShopCart(){
        // 1. 发请求,将产品添加到数据库(通知服务器)
        // 2. 服务器存储成功,进行路由跳转,并将产品信息以及购买产品的个数传递过来
        // 3. 失败,通知用户添加购物车失败
        let flag = this.$store.dispatch('addShopCart',{skuId:this.$route.params.skuid,skuNum: this.skuNum})
        flag.then(res =>{
          // this.$bus.$emit('INFO',this.skuInfo);
          // 在路由跳转时将商品数据传递给要跳转的路由组件(简单的数据可以以query的形式传递,复杂的数据则需要利用本地存储与会话存储实现H5新增)
